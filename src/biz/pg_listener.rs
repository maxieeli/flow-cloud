...
impl PgListeners {
    pub async fn new(pg_pool: &PgPool) -> Result<Self, Error> {
        ...
        let workspace_member_listener = WorkspaceMemberListener::new(pg_pool, "af_workspace_member_channel").await?;
        let collab_member_listener = CollabMemberListener::new(pg_pool, "af_collab_member_channel").await?;
        Ok(Self {
            user_listener,
