...
use database::collab::{
    batch_select_collab_blob, delete_collab, insert_into_af_collab,
    is_collab_exists, select_blob_from_af_collab, DatabaseResult,
};
...
impl CollabDiskCache {
    ...
    pub async fn get_collab_encoded(
        ...
        loop {
            let result = select_blob_from_af_collab(&self.pg_pool, &params.collab_type, &params.object_id).await;
            match result {
                Ok(data) => {
