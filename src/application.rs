...
pub async fn init_state(config: &Config, rt_cmd_tx: RTCommandSender) -> Result<AppState, Error> {
    ...
    let appflowy_ai_client = appflowy_ai::client::AppFlowyAIClient::new(config.appflowy_ai.url.expose_secret());
    // Pg listeners
    ...
    let access_control = AccessControl::new(pg_pool.clone(), metrics.access_control_metrics.clone()).await?;
