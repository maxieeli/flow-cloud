    ...
    AC: RealtimeAccessControl,
{
    pub fn new(
        ...
        let group_sender_by_object_id: Arc<DashMap<String, GroupCommandSender<U>>> = Arc::new(Default::default());
...
pub async fn broadcast_client_collab_message<U>(
    ...
    object_id: String,
    collab_messages: Vec<ClientCollabMessage>,
    client_stream: &Arc<DashMap<U, CollabClientStream>>,
) where
    U: RealtimeUser,
{
    if let Some(client_stream) = client_stream.get(user) {
        trace!(
...
pub type EncodeCollabSender = tokio::sync::oneshot::Sender<Option<EncodeCollab>>;
pub enum RTCommand {
    GetEncodeCollab {
        object_id: String,
