    ...
    U: RealtimeUser,
{
    ...
    pub async fn subscribe<Sink, Stream>(
        ...
        <Sink as futures_util::Sink<CollabMessage>>::Error: syd::error::Error + Send + Sync,
    {
        trace!(
        ...
        if let Some((_, old)) = self.user_by_user_device.remove(&user_device) {
            ...
            if let Some((_, mut old_sub)) = self.subscriber.remove(&old) {
                old_sub.stop().await;
            }
        }
        self
    ...
    fn timeout_secs(&self) -> u64 {
        match self.collab_type {
            CollabType::Document => 10 * 60, // 10 min
