...
pub(crate) struct RefreshTokenAction {
    ...
    goture_client: Arc<gotrue::api::Client>,
}
...
impl Action for RefreshTokenAction {
    ...
    fn run(&mut self) -> Self::Future {
        ...
        let weak_gotrue_client = Arc::downgrade(&self.goture_client);
        Box::pin(async move {
            if let (Some(token), Some(goture_client)) = (weak_token.upgrade(), weak_gotrue_client.upgrade()) {
                let refresh_token = token
                    ...
                    .as_str()
                    .to_owned();
                let access_token_resp = goture_client
